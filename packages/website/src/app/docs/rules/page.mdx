...
| `config` | `NestjsDoctorConfig \| undefined` | User configuration (optional) |
...
| Property | Type | Description |
|----------|------|-------------|
| `schemaGraph` | `SchemaGraph` | Entity-relation data extracted from Prisma or TypeORM |
| `orm` | `string` | Detected ORM name (e.g. `"prisma"`, `"typeorm"`) |
| `report()` | `(diagnostic) => void` | Emit a diagnostic |
